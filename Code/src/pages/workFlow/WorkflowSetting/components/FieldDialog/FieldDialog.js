...
import {
  Input,
  Select,
} from "../../../../../components/HADynamicForm/Components/controless"
...
export default function FieldDialog(props) {
  ...
  const checkEL = (props) => {
    ...
    if (required && !data[fieldName]) {
      error = true
      message = (abbrFieldName ? abbrFieldName : label) + ' is required'
    }
  ...
  return (
    ...
    >
      ...
      <Content dividers={true} className={classes.content}>
        <Input
          id={'fieldName'}
          required={true}
          fieldName={'fieldName'}
