...
function EnhancedTable(props) {
  ...
  const handleDelete = () => {
    if (loading) return;
    setLoading(true)
    assignApi.deleteMany({ idList: selected}).then(() => {
  ...
  const handleDetail = (_, id) => {
    const path = {
      pathname:'/aaa-service/assign/detail/'+id,
    }
    history.push(path);
  }
  ...
  const handleUpdate = (_, id) => {
    const path = {
      pathname: '/aaa-service/assign/update/' + id,
    }
    history.push(path)
  }
  ...
  return (
    <React.Fragment>
        <EnhancedTableToolbar
          ...
          tableName='Assign'
          createPath='/aaa-service/assign/create'
                ...
                .map((row, index) => {
                  ...
                  return (
                    <TableRow
                      ...
                      <TableCell align="center">{row.tenant_group_mapping.tenant.name}</TableCell>
                      <TableCell align="center">{row.tenant_group_mapping.ad_group.name}</TableCell>
