...
function EnhancedTable(props) {
  ...
  const handleDelete = () => {
    if (loading) return;
    setLoading(true)
    expiryApi.deleteMany({ idList: selected}).then(() => {
  ...
  const handleDetail = (_, id) => {
    const path = {
      pathname:'/aaa-service/expiry/detail/'+id,
    }
    history.push(path);
  }
  ...
  const handleUpdate = (_, id) => {
    const path = {
      pathname: '/aaa-service/expiry/update/' + id,
    }
    history.push(path)
  }
  ...
  return (
    <React.Fragment>
        <EnhancedTableToolbar
          ...
          tableName='Assign'
          createPath='/aaa-service/expiry/create'
                ...
                .map((row, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell align="center">{row.assign.tenant_group_mapping.tenant.name}</TableCell>
                      <TableCell align="center">{row.assign.tenant_group_mapping.ad_group.name}</TableCell>
                      <TableCell align="center">{row.assign.role.label}</TableCell>
                      <TableCell align="center">{row.user.displayname}</TableCell>
                      <TableCell align="center">{row.expiryDate ? dayjs(new Date(row.expiryDate)).format('YYYY-MM-DD') : ''}</TableCell>
