...
export default function HASelect(props) {
  ...
  useEffect(() => {
    setNewValue(defaultValue && defaultValue.value ? defaultValue.value : '')
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < itemList.length; i++) {
      if (itemList[i][valueField] === newValue) {
        label = itemList[i][labelField]
        break
      }
    }
    onChange && onChange({ id, label, value: newValue })
        ...
        >
          ...
          </MenuItem>
          {
            itemList && itemList.map((el, i) => (
              <MenuItem key={el[labelField] + '__' + i} value={el[valueField]}>{el[labelField]}</MenuItem>
            ))
          }
        </Select>
      </div>
